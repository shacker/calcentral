...
=end
  ...
  def get_feed_internal
    ...
    @maincats = LinkCategory.where("top_level = ?", true)
  ...
  def get_subsections_for_nav(cat)
    ...
    subsects = LinkSection.where("link_main_cat_id = ?", cat.id).select(:link_sub_cat_id).uniq
    subsects.each do |subsection|
      categories.push({"id" => subsection.link_sub_cat.slug, "name" => subsection.link_sub_cat.name})
    end
  ...
  def get_cats_for_link(link)
    ...
    sections.each do |section|
      catlist = {"topcategory" => section.link_sub_cat.name, "subcategory" => section.link_page_cat.name}
